...
import com.ht.season.board.Search;
...
public class BoardController {
	...
	// 게시글목록
	@RequestMapping("sug")
	public ModelAndView sug(@RequestParam(required = false, defaultValue = "1") int page
			,@RequestParam(required = false, defaultValue = "1") int range
			,@RequestParam(required = false, defaultValue = "num") String searchType
		...
		int listCnt = boardService.listCnt(search);
		search.pageInfo(page,range,listCnt);
		
		List<BoardDTO> sug = boardService.listAll(search);
		// ModelAndView - 모델과 뷰
